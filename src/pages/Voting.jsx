...
const VotingPage = () => {
  ...
  const initiative = legislations.find((item) => item.id === initiativeId);

  // Check if user already voted
  useEffect(() => {
    if (!currentUser || !initiativeId) return;

    const voteRef = ref(database, `VOTES/${initiativeId}/${currentUser.uid}`);
  ...
  const handleSubmit = () => {
    if (!selectedVote || hasVoted || !currentUser) return;
  ...
  const chartData = {
    ...
    datasets: [
      {
        data: [voteCounts.upvotes, voteCounts.downvotes],
  ...
  return (
    <section className="voting-page">
      <h2>Vote & Feedback</h2>
      <p>Participate in shaping your neighborhood by sharing your opinion!</p>
      ...
      {initiative ? (
        <>
          ...
          <div className="vote-buttons">
            <button
              onClick={() => handleVoteSelect("upvotes")}
              className={`upvote-btn ${
                selectedVote === "upvotes" ? "active" : ""
              }`}
              disabled={hasVoted}
            >
              👍 Thumbs Up
            ...
            <button
              onClick={() => handleVoteSelect("downvotes")}
              className={`downvote-btn ${
                selectedVote === "downvotes" ? "active" : ""
              }`}
              disabled={hasVoted}
            >
              👎 Thumbs Down
        ...
        <>
          <h3>Please select an issue to vote on</h3>
          <button
            className="go-to-initiative-btn"
